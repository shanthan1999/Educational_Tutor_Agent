    ...
    with st.sidebar:
        ...
        if not st.session_state.exa_tool:
            ...
            with st.expander("🔧 Debug Information", expanded=False):
                ...
                tavily_key_env = os.environ.get("TAVILY_API_KEY")
                if tavily_key_env:
                    key_preview = tavily_key_env[:10] + "..." if len(tavily_key_env) > 10 else "[SHORT]"
                    st.write(f"TAVILY_API_KEY: {key_preview}")
                else:
                    st.write("TAVILY_API_KEY: ❌ Not found")
                
                # Check if running on Streamlit Cloud
                is_cloud = os.getenv("STREAMLIT_SHARING_MODE") or "streamlit.app" in os.getenv("HOSTNAME", "")
                st.write(f"Running on Streamlit Cloud: {'✅ Yes' if is_cloud else '❌ No'}")
                
                # Dataset debug info
                if st.session_state.qa_chain:
                    try:
                        from tutor_agent import load_scienceqa_dataset
                        test_dataset = load_scienceqa_dataset(5)  # Load 5 examples for testing
                        
                        if isinstance(test_dataset, list) and len(test_dataset) == 3:
                            st.write("📊 Dataset: ❌ Fallback (3 examples)")
                            st.write("Issue: ScienceQA dataset failed to load")
                        else:
                            st.write(f"📊 Dataset: ✅ ScienceQA ({len(test_dataset)} examples)")
                    except Exception as e:
                        st.write(f"📊 Dataset: ❌ Error checking ({str(e)[:50]}...)")
                else:
                    st.write("📊 Dataset: ⏳ Not loaded yet")
                
                # Tool debug info
                if st.session_state.exa_tool:
                    if hasattr(st.session_state.exa_tool, 'tavily_client'):
                        tavily_status = "✅ Active" if st.session_state.exa_tool.tavily_client else "❌ None"
                        st.write(f"Tavily Client: {tavily_status}")
                        
                        if hasattr(st.session_state.exa_tool, 'tavily_api_key'):
                            key_status = "✅ Set" if st.session_state.exa_tool.tavily_api_key else "❌ Missing"
                            st.write(f"API Key in Tool: {key_status}")
                    else:
                        st.write("Tool Type: Fallback")
